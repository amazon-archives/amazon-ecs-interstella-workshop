...
Add a line to put all the parameters into JSON format and write it to disk as build.json. The parameters in this build.json file should map 1:1 to the parameters in service.yaml
...
- **Template File: service.yaml** - *The filename of the template that you looked over earlier in this workshop*
...
1\. Create a new stage for testing in your pipeline
...
<details>
  ...
  Within the pre-build stage, we'll want to install cfn-nag. Then we'll want to use the cfn_nag_scan command to scan the service.yaml CloudFormation template. It should look like this:
  <pre>
  version: 0.2
...
**Template: MyAppBuild::service.yaml**
